...
calc_completeness_percent <- function(dtable){
    na.max <- .2
...
score_fname_list <- function(fname_list){
    ...
    for (i in 1:length(fname_list)){
        ...
        score <- calc_completeness_percent(tab)
...
```{r}
columwise_NA_percent <- function(frame){
  percent_missing <- c()
  for (i in 1:ncol(frame)){
  
  percent_missing[i] <- sum(is.na(frame[,i]))/length(frame[,i])
