...
```{r}
# tuition_in_state
tuition_in_state <- outcomes2013[,c("INSTNM","tuition_in_state")]
...
```{r}
# faculty_salary
faculty_salary <- outcomes2013[,c("INSTNM","faculty_salary")]
colnames(faculty_salary) <- c("School", "Value")
faculty_salary <-  as.data.frame(mutate(faculty_salary, Feature="faculty_salary"))
faculty_salary$School[which(faculty_salary$School != "New College of Florida")] = "Other schools"
ggplot(faculty_salary, aes(y=Value, x=Feature, group=School, fill=School)) + geom_boxplot(position=position_dodge(1))
```
