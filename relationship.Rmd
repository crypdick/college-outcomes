...
```{r}
# 
mid_SAT_math <- outcomes2013[,c("INSTNM","SATMTMID")]
colnames(mid_SAT_math) <- c("School", "Value")
mid_SAT_math <-  as.data.frame(mutate(mid_SAT_math, Feature="mid_SAT_math"))
mid_SAT_math$School[which(mid_SAT_math$School != "New College of Florida")] = "Other schools"
ggplot(mid_SAT_math, aes(y=Value, x=Feature, group=School, fill=School)) + geom_boxplot(position=position_dodge(1))
```
