...
female_percent <- mutate(female_percent, Feature="Percnt_female")

percent_recv_loans <- outcomes2013[,c("INSTNM","PCTFLOAN")]
percent_recv_loans <- mutate(percent_recv_loans, Feature="Percnt_reciv_loan")

percnt_older_than_25yr <- outcomes2013[,c("INSTNM","UG25ABV")]
percnt_older_than_25yr <- mutate(percnt_older_than_25yr, Feature="Percnt_older_than_25yr")

Avg_fam_income <- outcomes2013[, c("INSTNM","FAMINC")]
Avg_fam_income <- mutate(Avg_fam_income, Feature="Avg_fam_income")

mean_income_graduates_after_8yr <- outcomes2013[, c("INSTNM","MNEARN8")]
mean_income_graduates_after_8yr <- mutate(mean_income_graduates_after_8yr, Feature="Mean_income_graduates_after_8yr")

#Share of students earning over $25,000/year (threshold earnings) 8 years after entry
perc_graduates_earning_grtrthn25k_peryer <- outcomes2013[,c("INSTNM","GT25K")]
perc_graduates_earning_grtrthn25k_peryer <-  mutate(perc_graduates_earning_grtrthn25k_peryer, Feature="Perc_graduates_earning_grtrthn25k_peryer")



sample$INSTNM[which(sample$INSTNM != "New College of Florida")] = "Other schools"
